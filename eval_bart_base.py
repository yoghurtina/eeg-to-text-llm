...
# from model_bart_base import BrainTranslator, BrainTranslatorNaive
...
#         model = BrainTranslator(pretrained_bart, in_feature = 105*len(bands_choice), decoder_embedding_size = 768, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048)
#     elif model_name == 'BrainTranslatorNaive':
#         model = BrainTranslatorNaive(pretrained_bart, in_feature = 105*len(bands_choice), decoder_embedding_size = 768, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048)
...
if __name__ == '__main__':
    ...
    # Instead of eval, directly convert the arguments
    teacher_forcing = bool(int(args['tf']))
    intput_noise = bool(int(args['noise']))
    print(f'teacher_forcing: {teacher_forcing}, intput_noise: {intput_noise}')

    output_all_results_path = (
        f'./results/{task_name}-{model_name}{"-teacher_forcing" if teacher_forcing else ""}'
        f'{"-intput_noise" if intput_noise else ""}-all_decoding_results.txt'
    )
    output_all_metrics_results_path = output_all_results_path.replace('txt', 'json')
